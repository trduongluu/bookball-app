...
@Component({
  selector: 'app-storage-cabinets',
  templateUrl: './storage-cabinets.component.html',
  styleUrls: ['./storage-cabinets.component.scss']
})
export class StorageCabinetsComponent extends BaseListComponent implements OnInit {
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    if (formSearch.txtSearch) {
      if (!isNaN(+formSearch.txtSearch)) {
        where.and.push({
          or: [
            { 'data.cabCode': formSearch.txtSearch }
          ]
        });
      } else {
        where.and.push({
          or: [
            // { 'data.cabCode': { like: this.ex.BoDau(formSearch.txtSearch) } },
            { 'data.cabinetName': { like: this.ex.BoDau(formSearch.txtSearch) } },
            { 'companyInfo.companyName': { like: this.ex.BoDau(formSearch.txtSearch) } },
            { 'employyes.basic.fullName': { like: this.ex.BoDau(formSearch.txtSearch) } }
          ]
        });
      }
    }
