...
export class PartnerComponent extends BaseListComponent implements OnInit {
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    if (formSearch.txtSearch) {
      if (!isNaN(+formSearch.txtSearch)) {
        where.and.push({
          or: [
            { 'data.partCode': { like: this.ex.BoDau(formSearch.txtSearch) } },
          ]
        });
      } else {
        where.and.push({
          or: [
            { 'data.partCode': { like: this.ex.BoDau(formSearch.txtSearch) } },
            { 'data.partnerNameVn': { like: this.ex.BoDau(formSearch.txtSearch) } },
            { 'partnerGroup.data.partnerGroupNameVn': { like: this.ex.BoDau(formSearch.txtSearch) } }
          ]
        });
      }
    }
