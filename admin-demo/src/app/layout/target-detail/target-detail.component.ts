...
export class TargetDetailComponent extends BaseListComponent implements OnInit {
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    const where: any = { and: [{ 'data.idtarget': null }] };
    ...
    if (formSearch.txtSearch) {
      where.and.push({
        or: [
          { 'data.targetDetailNameVn': { like: this.ex.BoDau(formSearch.txtSearch) } },
          // { 'data.stageIndex': +formSearch.txtSearch },
          // { 'data.stagePercent': +formSearch.txtSearch },
          // { 'data.idtarget': +formSearch.txtSearch }
        ]
      });
    }
    ...
    if (!!!formSearch.txtSearch && formSearch.status === null) {
      where.and.push({ 'data.idtarget': null });
    }
  ...
  checkShowProcess() {
    for (const x of this.listOfData) {
      ...
      if (x === this.listOfData[this.listOfData.length - 1] && this.isShowProcess && !x.child[0]) {
  ...
  async loadChild(item: any) {
    ...
    if (item.show) {
      const formSearch = this.myForm.value;
      const where: any = { and: [{ 'data.idtarget': item.id }] };
      ...
      if (rs.ok) {
        if (JSON.stringify(rs.result.data) !== '[]') {
          ...
          this.message.warning(`${item.data.targetDetailNameVn} không có mục tiêu con nào`);
        }
      }
    }
  }
