...
import { ManagementFormService } from 'src/app/_shared/services/management-form.service';

@Component({
  selector: 'app-management-form-data',
  templateUrl: './management-form-data.component.html',
  styleUrls: ['./management-form-data.component.scss']
})
export class ManagementFormDataComponent extends BaseDataComponent implements OnInit {

  constructor(
    fb: FormBuilder,
    private ex: ExtensionService,
    private sv: ManagementFormService,
  ...
  async ngOnInit() {
    this.createForm({
      data: this.fb.group({
        mntFormName: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
        mntFormIndex: [{ value: null, disabled: this.isView }],
