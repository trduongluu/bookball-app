...
export class ManagementFormComponent extends BaseListComponent implements OnInit {
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    if (formSearch.txtSearch) {
      if (!isNaN(+formSearch.txtSearch)) {
        where.and.push({
          or: [
            { 'data.mntFormIndex': formSearch.txtSearch },
          ]
        });
      } else {
        where.and.push({
          or: [
            { 'data.mntFormName': { like: this.ex.BoDau(formSearch.txtSearch) } },
          ]
        });
      }
    }
