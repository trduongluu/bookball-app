...
import { FieldOfActivityService } from 'src/app/_shared/services/field-of-activity.service';


@Component({
  selector: 'app-field-of-activity-data',
  templateUrl: './field-of-activity-data.component.html',
  styleUrls: ['./field-of-activity-data.component.scss']
})
export class FieldOfActivityDataComponent extends BaseDataComponent implements OnInit {

  constructor(
    fb: FormBuilder,
    private ex: ExtensionService,
    private sv: FieldOfActivityService,
  ...
  async ngOnInit() {
    this.createForm({
      data: this.fb.group({
        fieldOfActivityName: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
        activityIndex: [{ value: null, disabled: this.isView }],
