...
export class FieldOfActivityComponent extends BaseListComponent implements OnInit {
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    if (formSearch.txtSearch) {
      if (!isNaN(+formSearch.txtSearch)) {
        where.and.push({
          or: [
            { 'data.activityIndex': formSearch.txtSearch },
        ...
        where.and.push({
          or: [
            { 'data.fieldOfActivityName': { like: formSearch.txtSearch } },
          ]
        });
      }
    }
