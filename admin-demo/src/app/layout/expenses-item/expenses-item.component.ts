...
export class ExpensesItemComponent extends BaseListComponent implements OnInit {
  ...
  async getExpItemName() {
    const rs = await this.sv.get({});
    if (rs.ok) {
      rs.result.data.forEach(x => {
        this.listOfData.forEach(a => {
          if (a.data.expItemGroup === x.id) {
            a.data.expItemGroup = x.data.expensesItemName;
          }
        });
      });
    }
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    if (formSearch.txtSearch) {
      where.and.push({
        or: [
          { 'data.expensesItemName': { like: this.ex.BoDau(formSearch.txtSearch) } }
        ]
      });
    }
    ...
    this.ex.logDebug('getData', rs.result);
    if (rs.ok) {
  ...
  //     where.and.push({
  //       or: [
  //         { 'data.expItemGroup': { like: this.ex.BoDau(formSearch.txtSearch) } },
  //         { 'data.expensesItemName': { like: this.ex.BoDau(formSearch.txtSearch) } },
  //         { 'data.expensesItemIndex': { like: this.ex.BoDau(formSearch.txtSearch) } },
  //       ]
  //     });
  //   }
  //   if (formSearch.status !== null) {
  ...
  //   if (rs.ok) {
  //     this.listOfData = rs.result.data;
  //     this.listOfData.forEach(x => x.isHidden = x.dataDb.status === 1);
  //     this.paging = rs.result.paging;
