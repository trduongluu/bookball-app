...
@Component({
  selector: 'app-issue-type-data',
  templateUrl: './issue-type-data.component.html',
  styleUrls: ['./issue-type-data.component.scss']
})
export class IssueTypeDataComponent extends BaseDataComponent implements OnInit {

  constructor(
    fb: FormBuilder,
    private ex: ExtensionService,
    private sv: IssueTypeService,
  ...
  async ngOnInit() {
    this.createForm({
      data: this.fb.group({
        issueTypeName: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
        issueTypeIndex: [{ value: null, disabled: this.isView }],
