...
@Component({
  selector: 'app-expenses-item-group',
  templateUrl: './expenses-item-group.component.html',
  styleUrls: ['./expenses-item-group.component.scss']
})
export class ExpensesItemGroupComponent extends BaseListComponent implements OnInit {
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    if (formSearch.txtSearch) {
      if (!isNaN(+formSearch.txtSearch)) {
        ...
        where.and.push({
          or: [
            { 'data.expensesItemGroupName': { like: this.ex.BoDau(formSearch.txtSearch) } }
          ]
        });
      }
    }
