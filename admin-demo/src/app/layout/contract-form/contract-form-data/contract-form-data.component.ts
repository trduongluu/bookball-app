...
@Component({
  selector: 'app-contract-form-data',
  templateUrl: './contract-form-data.component.html',
  styleUrls: ['./contract-form-data.component.scss']
})
export class ContractFormDataComponent extends BaseDataComponent implements OnInit {

  constructor(
    fb: FormBuilder,
    private ex: ExtensionService,
    private sv: ContractFormService,
  ...
  async ngOnInit() {
    this.createForm({
      data: this.fb.group({
        contractFormName: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
        contractFormIndex: [{ value: null, disabled: this.isView }],
