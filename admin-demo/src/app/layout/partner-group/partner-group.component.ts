...
@Component({
  selector: 'app-partner-group',
  templateUrl: './partner-group.component.html',
  styleUrls: ['./partner-group.component.scss']
})
export class PartnerGroupComponent extends BaseListComponent implements OnInit {
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    if (formSearch.txtSearch) {
      if (!isNaN(+formSearch.txtSearch)) {
        ...
        where.and.push({
          or: [
            { 'data.partnerGroupNameVn': { like: this.ex.BoDau(formSearch.txtSearch) } }
          ]
        });
      }
    }
