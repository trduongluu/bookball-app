...
@Component({
  selector: 'app-project-profile-data',
  templateUrl: './project-profile-data.component.html',
  styleUrls: ['./project-profile-data.component.scss']
})
export class ProjectProfileDataComponent extends BaseDataComponent implements OnInit {

  constructor(
    fb: FormBuilder,
    private ex: ExtensionService,
    private sv: ProjectProfileService,
  ...
  async ngOnInit() {
    this.createForm({
      data: this.fb.group({
        profileCode: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
        profileNameVn: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
