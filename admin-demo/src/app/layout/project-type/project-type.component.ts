...
export class ProjectTypeComponent extends BaseListComponent implements OnInit {
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    if (formSearch.txtSearch) {
      console.log('formSearch.txtSearch', this.ex.BoDau(formSearch.txtSearch));
      ...
      if (!isNaN(+formSearch.txtSearch)) {
        ...
        where.and.push({
          or: [
            { 'data.ProjTypeNameVn': { like: this.ex.BoDau(formSearch.txtSearch) } }
          ]
        });
      }
    }
