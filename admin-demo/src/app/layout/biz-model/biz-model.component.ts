...
export class BizModelComponent extends BaseListComponent implements OnInit {
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    if (formSearch.txtSearch) {
      where.and.push({
        or: [
          { 'data.BizModeCode': { like: this.ex.BoDau(formSearch.txtSearch) } },
          { 'data.BizModeNameVn': { like: this.ex.BoDau(formSearch.txtSearch) } },
          { 'data.BinddingModelIndex': { like: this.ex.BoDau(formSearch.txtSearch) } },
        ]
      });
    }
