...
import { InvestsService } from 'src/app/_shared/services/invests.service';

@Component({
  selector: 'app-invests-data',
  templateUrl: './invests-data.component.html',
  styleUrls: ['./invests-data.component.scss']
})
export class InvestsDataComponent extends BaseDataComponent implements OnInit {

  constructor(
    fb: FormBuilder,
    private ex: ExtensionService,
    private sv: InvestsService,
  ...
  async ngOnInit() {
    this.createForm({
      data: this.fb.group({
        investName: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
        investIndex: [{ value: null, disabled: this.isView }],
