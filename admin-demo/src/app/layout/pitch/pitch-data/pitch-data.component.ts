...
import { PitchService } from 'src/app/_shared/services/pitch.service';
import { NzMessageService } from 'ng-zorro-antd/message';
import { GlobalValidate } from 'src/app/_base/class/global-validate';

@Component({
  selector: 'app-pitch-data',
  templateUrl: './pitch-data.component.html',
  styleUrls: ['./pitch-data.component.scss']
})
export class PitchDataComponent extends BaseDataComponent implements OnInit {

  public listPositionType = [];
  constructor(
    fb: FormBuilder,
    private ex: ExtensionService,
    private sv: PitchService,
  ...
  async ngOnInit() {
    this.createForm({
      data: this.fb.group({
        name: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
        address: [{ value: null, disabled: this.isView }],
        description: [{ value: null, disabled: this.isView }],
      }),
