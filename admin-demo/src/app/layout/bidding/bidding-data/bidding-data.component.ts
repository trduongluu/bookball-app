...
@Component({
  selector: 'app-bidding-data',
  templateUrl: './bidding-data.component.html',
  styleUrls: ['./bidding-data.component.scss']
})
export class BiddingDataComponent extends BaseDataComponent implements OnInit {

  constructor(
    fb: FormBuilder,
    private ex: ExtensionService,
    private sv: BiddingService,
  ...
  async ngOnInit() {
    this.createForm({
      data: this.fb.group({
        biddingName: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
        biddingIndex: [{ value: null, disabled: this.isView }, [GlobalValidate.MinLengthNumber(0, { error: 'Không được nhỏ hơn' })]],
