...
export class StatusDataComponent extends BaseDataComponent implements OnInit {
  ...
  async ngOnInit() {
    this.createForm({
      data: this.fb.group({
        statusName: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
        statusType: [{ value: null, disabled: this.isView }],
  ...
  async getlistStatusType() {
    const params: PagingModel = {
      page: 1,
      size: 100,
      where: { and: [{ 'data.statusType': 0 }] }
    };
    const rs = await this.sv.get(params);
    console.log('status', rs.result);
    if (rs.ok) {
      this.listStatusType = rs.result.data.map(x => {
        return {
          id: x.id,
