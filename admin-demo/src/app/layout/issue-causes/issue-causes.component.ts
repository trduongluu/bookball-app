...
@Component({
  selector: 'app-issue-causes',
  templateUrl: './issue-causes.component.html',
  styleUrls: ['./issue-causes.component.scss']
})
export class IssueCausesComponent extends BaseListComponent implements OnInit {
  ...
  async getData(page: number = 1, order: any[] = [{ id: false }]) {
    ...
    if (formSearch.txtSearch) {
      if (!isNaN(+formSearch.txtSearch)) {
        where.and.push({
          or: [
            { 'data.issueId': formSearch.txtSearch }
          ]
        });
      } else {
        where.and.push({
          or: [
            { 'data.issueCausesName': { like: this.ex.BoDau(formSearch.txtSearch) } }
          ]
        });
      }
    }
