...
@Component({
  selector: 'app-issue-causes-data',
  templateUrl: './issue-causes-data.component.html',
  styleUrls: ['./issue-causes-data.component.scss']
})
export class IssueCausesDataComponent extends BaseDataComponent implements OnInit {

  constructor(
    fb: FormBuilder,
    private ex: ExtensionService,
    private sv: IssueCausesService,
  ...
  async ngOnInit() {
    this.createForm({
      data: this.fb.group({
        issueCausesName: [{ value: null, disabled: this.isView }, [GlobalValidate.required({ error: 'Không được để trống' })]],
        issueCausesIndex: [{ value: null, disabled: this.isView }],
